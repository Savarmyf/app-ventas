...
dia = st.text_input("Fecha (DD/MM)", value=datetime.now().strftime("%d/%m"))
cantidad = st.number_input("Contactos de hoy", min_value=0, step=1)

if st.button("Guardar registro"):
    registros.setdefault(usuario, [])
    existe = any(r["dia"] == dia for r in registros[usuario])

    if existe:
        st.warning("⚠️ Ya cargaste un registro para ese día")
    else:
        registros[usuario].append({"dia": dia, "cantidad": cantidad})
...
if mis_registros:
    ...
    st.metric("Total de contactos", total)
    st.metric("Promedio diario", round(promedio, 2))
    ...
    # Racha simple
    dias = [r["dia"] for r in mis_registros]
    racha = 1
    ...
    st.write(f"🔥 Tu racha actual: **{racha} días**")
else:
...
# -------------------- Ranking equipo (opcional visible) --------------------
